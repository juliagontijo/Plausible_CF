...
def dijsktra_tosome2(graph, initial, targets):
    paths = {}; dists = {}
    #dijsktra_partial = partial(dijsktra, graph, initial)
    for target in targets:
        sat = dijsktra(graph, initial, target)
        dist, path = sat
        paths[target] = path
        dists[target] = dist
    return dists, paths
...
def dijsktra_to_some_final_nodes(graph, initial, final_nodes_df):
    import math
    edges = graph.edges
    weights = graph.weights
    
    # Assuming 'node_id' is the column in final_nodes_df that lists the node identifiers
    final_nodes = set(final_nodes_df.index)

    nodes_status = {node: [math.inf, 'T'] for node in edges.keys() if node != initial}
    nodes_status[initial] = [0, 'P']

    previous_nodes = {}
    current_node = initial
    processed_final_nodes = set()

    while True:
        distance_to_current_node = nodes_status[current_node][0]
        neighbours = edges[current_node]
        for neighbour in neighbours:
            if (current_node, neighbour) in weights and neighbour in nodes_status and nodes_status[neighbour][1] == 'T':
                current_distance = nodes_status[neighbour][0]
                traversal_cost = weights[(current_node, neighbour)]
                proposed_distance = distance_to_current_node + traversal_cost
                if proposed_distance < current_distance:
        ...
        # Break the loop if all final nodes have been processed
        if final_nodes.issubset(processed_final_nodes):
            break

        filtered_weights = {node: status[0] for node, status in nodes_status.items() if status[1] == 'T'}
        if not filtered_weights:
            break

        best_node = min(filtered_weights, key=filtered_weights.get)
        nodes_status[best_node][1] = 'P'
        processed_final_nodes.add(best_node)
        current_node = best_node

    paths = {node: return_path(previous_nodes, node, [node]) for node in final_nodes if node in previous_nodes}

    return nodes_status, paths

def return_path(previous_nodes, node, path):
    """Recursive function to return the path from previous_nodes dictionary."""
    if node not in previous_nodes:
        return path[::-1]  # Return reversed path
    return return_path(previous_nodes, previous_nodes[node], path + [previous_nodes[node]])
